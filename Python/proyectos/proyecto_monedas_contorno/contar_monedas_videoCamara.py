...
while True:
    ...
    imagen_A4 = alineamientoVideo( videoCamara, 720, 1080)

    if imagen_A4 is not None:
        puntos = []
        valorMatrizGauss = 5

        videoEscalaGrises = cv2.cvtColor( imagen_A4, cv2.COLOR_RGB2GRAY)

        videoFiltroGaussiano = cv2.GaussianBlur( videoEscalaGrises, (valorMatrizGauss,valorMatrizGauss), 1)
        ...
        contorno , jerarquia = cv2.findContours( videoUmbral, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]

        cv2.drawContours( image=imagen_A4 , contours=contorno , contourIdx=-1 , color=(0,0,255))
        ...
        for c in contorno:
            ...
            if areaMoneda < 10200 and areaMoneda > 9500:
